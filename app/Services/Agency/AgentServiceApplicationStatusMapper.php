...
class AgentServiceApplicationStatusMapper
{
    ...
    private function mapEnergyStatus(string $serviceType, ?int $status): void
    {
        $value = match ($status) {
            ...
            ConnectionService::STATUS_EA_PROCESSINF => ['status_name' => 'Awaiting Confirmation', 'description' => 'The connection is processed and we are awaiting confirmation from the provider.'],
            default => self::DEFAULT_STATUS
        };
    ...
    private function mapWaterStatus(string $serviceType, ?int $status): void
    {
        $value = match ($status) {
            ...
            ConnectionService::STATUS_EA_PROCESSINF,
