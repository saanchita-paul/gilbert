...
//        $existingApplication->status = ConnectionApplication::STATUS_MAPPING[$application['status']];
     ...
     * @param int $applicationId
     */
    public function checkTriageFlag(int $applicationId)
    {
        // get data
        $details = $this->getData($applicationId);

        return $details->source === ConnectionApplication::SOURCE_HOOD_LEAD ? $this->checkIfHoodLead($details) : $this->checkIfNotHoodLead($details);
    }

    private function getData($applicationId){
        $applicationDetails = ConnectionApplication::query()
            ->select('id', 'title', 'first_name', 'last_name', 'dob', 'phone', 'email', 'moving_date', 'nmi', 'source', 'address_text', 'street_address', 'street_number', 'city', 'postcode', 'state', 'country')
            ->where('id', $applicationId)
            ->with(['identification' => function($query){
                $query->select('id', 'connection_application_id', 'type', 'card_number');
            }])
            ->first();
        return $applicationDetails;
    }

    private function checkIfHoodLead($applicationDetails){
        if (is_null($applicationDetails->title)){
            return true;
        }
        if (is_null($applicationDetails->first_name)){
            return true;
        }
        if (is_null($applicationDetails->last_name)){
            return true;
        }
        if (is_null($applicationDetails->phone)){
            return true;
        }
        if (is_null($applicationDetails->email)){
            return true;
        }
    }

    private function checkIfNotHoodLead($applicationDetails){
        if (is_null($applicationDetails->title)){
            return true;
        }
        if (is_null($applicationDetails->first_name)){
        ...
        if (is_null($applicationDetails->dob)){
            return true;
        }
        if (is_null($applicationDetails->phone)){
            return true;
        ...
        if (is_null($applicationDetails->moving_date)){
            return true;
        }
        if (is_null($applicationDetails->nmi)){
            return true;
        }
        if (is_null($applicationDetails->identification->type)){
            return true;
        }
        if (is_null($applicationDetails->identification->card_number)){
            return true;
        }
    }
