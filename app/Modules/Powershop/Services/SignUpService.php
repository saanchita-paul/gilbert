...
class SignUpService
{
    ...
    private function getVulnerabilities()
    {
        ...
        if ($this->application->is_gas_life_support || $this->application->is_power_life_support) {
            ...
            $data['medical_reason'] = $this->getMedicalReason();
            $data['life_support_equipment'] = $this->application->lifeSupportEquipment->powershop_value;
        }
    ...
    private function getMedicalReason()
    {
        return $this->application->medical_reason ?? null;
    }
}
