    ...
    <v-row v-if="leadSummary.id">
        <v-col cols="12" class="d-flex justify-space-between">
            <div class="d-flex">
                <div class="mx-4 mb-0">
                    <p class="page-title mb-0">
                        <span>
                            <v-img @click="goToBack()" src="/assets/images/icons/back_btn.png" max-height="40px"
                                   max-width="40px" class="back-btn mt-1">
                            </v-img>
                        </span>
                        <span>
            ...
            <div>
                ...
                <p v-if="leadSummary.is_contacted" class="application-consent mt-5">
                    <v-icon size="14px" color="success" class="mx-2">call</v-icon>
                    Applicant consents to be contacted by HOOD
                </p>
            </div>
        </v-col>
        ...
        <div style="width: 100%;" class="mb-4 ml-6 mr-4 pl-2">
            <div class="d-flex justify-space-between" style="width: 100%;">
                <div class="d-flex">
                    ...
                    <div class="d-flex justify-center" style="flex-wrap: wrap;">
                        <div style="flex-basis: 100%; text-align: center;">
                            <div class="font-weight-bold" :class="{'mx-1':isEnergyActive('Power')}">
                                <v-icon size="16" :color="getEnergyColor('Power')">mdi-flash</v-icon>
                                Power
                            </div>
                        </div>
                        <div :style="{ 'text-align': 'center', color: getEnergyServiceStatus('power').color }">
                            {{ getEnergyServiceStatus('power').text }}
                        </div>
                    </div>

                    <div class="d-flex justify-center" style="flex-wrap: wrap;">
                        <div style="flex-basis: 100%; text-align: center;">
                            <div class="font-weight-bold" :class="{'mx-1':isEnergyActive('Gas')}">
                                <v-icon size="16" :color="getEnergyColor('Gas')">mdi-fire</v-icon>
                                Gas
                            </div>
                        </div>
                        <div :style="{ 'text-align': 'center', color: getEnergyServiceStatus('gas').color }">
                            {{ getEnergyServiceStatus('gas').text }}
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-center" style="flex-wrap: wrap;">
                        <div style="flex-basis: 100%; text-align: center;">
                            <div class="font-weight-bold" :class="{'mx-1':isActive('Water')}">
                                <v-icon size="16" :color="getColor('Water')">mdi-water</v-icon>
                                Water
                            </div>
                        </div>
                        <div :style="{ 'text-align': 'center', color: getServiceStatus('water').color }">
                            {{ getServiceStatus('water').text }}
                        </div>
                    </div>

                    <div class="d-flex justify-center" style="flex-wrap: wrap;">
                        <div style="flex-basis: 100%; text-align: center;">
                            <div class="font-weight-bold" :class="{'mx-1':isActive('Internet')}">
                                <v-icon size="16" :color="getColor('Internet')">mdi-wifi</v-icon>
                                Internet
                            </div>
                        </div>
                        <div :style="{ 'text-align': 'center', color: getServiceStatus('internet').color }">
                            {{ getServiceStatus('internet').text }}
                        </div>
                    </div>
                ...
                <div class="d-flex align-end">
                    <span class="font-weight-bold">Application Status: </span> <span
                    class="grey--text pl-2"> {{ leadSummary.status }} </span>

                    <span class="font-weight-bold ml-4">Payment Status: </span> <span
                    class="grey--text pl-2"> {{ getPaymentStatus }} </span>
                </div>
            </div>
        </div>
        ...
        <v-col cols="12" class="mt-n6">
            <div class="d-flex justify-space-between ">
                <div class="d-flex">
                    <div class="ml-4"><span class="font-weight-bold text-sm">Agent Name:</span>
                        <span>{{ this.leadSummary.agent_name }}</span></div>
                    <p class="ml-4"><span class="font-weight-bold">Agency:</span>
                        <span>{{ this.leadSummary.agency_office }}</span></p>
                </div>
                <div class="d-flex" v-if="this.leadSummary.source == this.leadSourceMap['SOURCE_FOXIE']">
                    <p class="ml-4"><span class="font-weight-bold">Lead Source:</span>
                        <span>{{ this.leadSummary.lead_source }}</span></p>
                    <p class="ml-4"><span class="font-weight-bold">LS Description:</span>
                        <span>{{ this.leadSummary.lead_source_description }}</span></p>
                </div>
            </div>
        </v-col>
        <v-col cols="12">
